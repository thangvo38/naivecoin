...
class Blockchain {
    constructor(dbName) {
        ...
        // INFO: In this implementation the database is a file and every time data is saved it rewrites the file, \
        // probably it should be a more robust database for performance reasons
    ...
    getAccumulatedDifficulty(aBlockchain) {
        return aBlockchain
            .map((block) => bigInt(block.getDifficulty().toString()))
            .map((difficulty) => difficulty.pow(2))
            .reduce((a, b) => a.add(b));
    }
    ...
    replaceChain(newBlockchain) {
        ...
        // Choose the chain with the higher cumulative difficulty
        // console.assert(this.getAccumulatedDifficulty(newBlockchain), this.getAccumulatedDifficulty(this.blocks));
        // if (this.getAccumulatedDifficulty(newBlockchain) > this.getAccumulatedDifficulty(this.blocks)) {

        if (this.getAccumulatedDifficulty(newBlockchain).compare(this.getAccumulatedDifficulty(this.blocks)) === 1)  {
    ...
    //[Thang Vo] Need to be modified like naivecoin
    ...
    checkBlock(newBlock, previousBlock, referenceBlockchain = this.blocks) {
        ...
        } else if (newBlock.getDifficulty() >= this.getDifficulty(newBlock.index)) { // [Thang Vo] If the difficulty level of the proof-of-work challenge is correct
        ...
        // INFO: Here it would need to check if the block follows some expectation regarging the minimal number of transactions, 
        // value or data size to avoid empty blocks being mined.
